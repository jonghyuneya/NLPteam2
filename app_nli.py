...
from modules.nli_multilabel_final import apply_nli_multilabel_for_business  # NLI로 변경
...
def create_kpi_cards(summary_df: pd.DataFrame, sentences_df: pd.DataFrame) -> None:
    ...
    with col2:
        st.metric(
            label="🎯 Multi-Label Aspects", 
            value=total_topics,
            help="Number of distinct multi-label aspects identified"
        )
...
def parse_multiple_labels(label_str: str) -> List[str]:
    """Parse underscore-separated multiple labels from NLI classification"""
...
def create_topic_overview_chart(summary_df: pd.DataFrame) -> None:
    ...
    fig = make_subplots(
        rows=1, cols=2,
        subplot_titles=('Multi-Label Performance Overview (All Aspects)', 'Aspect Volume Distribution'),
    ...
    fig.update_layout(
        height=600,
        showlegend=False,
        title_text="NLI Multi-Label Aspect Analysis Overview",
        title_x=0.5,
        title_font_size=16
    )
    ...
    with st.expander("🏷️ Comprehensive Multi-Label Analysis", expanded=True):
        st.subheader("All NLI Classifications by Aspect")
        ...
        for _, row in df.sort_values('n', ascending=False).head(10).iterrows():
            ...
            with st.container():
                ...
                with col1:
                    st.markdown(f"**Aspect {row['topic_id']}: {len(labels)} Labels**")
                with col2:
                ...
                st.markdown("**All NLI Classifications:**")
...
def create_priority_matrix(summary_df: pd.DataFrame) -> None:
    ...
    fig.update_layout(
        title="Multi-Aspect Priority Matrix",
...
def create_sentiment_timeline(sentences_df: pd.DataFrame, summary_df: pd.DataFrame) -> None:
    ...
    with col1:
        num_topics = st.selectbox("Number of aspects to show", [2, 3, 4, 5], index=1, key="timeline_topics")
    ...
    if not monthly_metrics:
        st.info(f"Not enough data for timeline analysis. Try reducing minimum reviews to {min_reviews//2} or fewer aspects.")
        return
    ...
    fig.update_layout(
        title={
            'text': f"Sentiment Trends - Top {num_topics} Aspects ({time_grouping})",
            'x': 0.5,
            'font': {'size': 16}
        },
    ...
    with st.expander("ℹ️ Data Quality Info"):
        ...
        st.write(f"- Showing top {num_topics} aspects by volume")
        st.write(f"- Time grouping: {time_grouping}")
        st.write(f"- Minimum {min_reviews} reviews per period")
        st.write(f"- Minimum 3 data points per aspect")
        st.write(f"- Data points per aspect:")
        ...
        st.write(f"- Total valid data points: {len(combined_timeline)} period-aspect combinations")
...
def generate_enhanced_llm_insights(summary_df: pd.DataFrame, sentences_df: pd.DataFrame) -> str:
    ...
    # Enhanced prompt 구성
    enhanced_prompt = f"""
You are analyzing customer feedback using advanced NLI-based multi-label classification. 

=== CRITICAL UNDERSTANDING GUIDELINES ===

**IMPORTANT - NLI MULTI-LABEL INTERPRETATION:**
- Each aspect represents a HOLISTIC customer experience area with multiple interconnected components
- Don't treat the NLI labels as separate issues - they are facets of the same unified experience
...
- An aspect labeled "order accuracy (missing items)" with 85% positive rate means customers are SATISFIED with order accuracy
- Focus on how improving one component creates positive ripple effects across related components within the same aspect
...
- Multiple components in one aspect = Opportunity for holistic improvement
- Cross-aspect patterns = Systemic operational insights

=== BUSINESS OVERVIEW ===
- Total Reviews Analyzed: {total_reviews:,}
- Average Rating: {avg_stars:.1f}/5.0 stars
- Overall Positive Sentiment: {positive_rate:.1%}
- Multi-Label Aspects Identified: {len(df)}

=== DETAILED ASPECT ANALYSIS WITH REAL EXAMPLES ===
"""
...
2. **Performance Reality Check**
   - Aspects that LOOK concerning but actually perform well (high scores despite concerning labels)
   - Aspects that LOOK fine but need attention (low scores despite neutral labels)
   - Use specific customer quotes as evidence

3. **Systemic Experience Insights** 
   - How do the multi-aspect topics interconnect?
   - Which single improvements would create the biggest ripple effects across multiple aspects?
   - Cross-aspect improvement opportunities
...
6. **Customer Experience Journey Insights**
   - How different aspects connect in the customer journey
   - Critical moments that impact multiple aspects simultaneously

REMEMBER: 
- An aspect about "wait time (long delays)" with 80% positive rate means wait times are GOOD, not bad!
...
"""
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You are an expert business intelligence analyst specializing in customer experience and restaurant operations. You have deep understanding of NLI-based multi-label classification and how it reveals interconnected aspects of customer experience. You always base recommendations on actual performance data, not label assumptions."},
...
def format_aspect_breakdown(raw_label, positive_rate):
    ...
    interpretation = f"This aspect covers {len(aspects)} interconnected components. With {positive_rate:.1%} positive rate, this indicates "
    if positive_rate > 0.7:
        interpretation += "STRONG performance across all these connected components:"
    elif positive_rate > 0.4:
        interpretation += "MIXED performance - some components working well, others need attention:"
    else:
...
Key Aspect Trends:
{chr(10).join(topic_trends) if topic_trends else "Insufficient data for aspect-level trends"}
...
{chr(10).join(complexity_analysis) if complexity_analysis else "No highly complex aspects found"}

Performance Clustering:
- Excellent (>80% positive): {excellent_topics} aspects
- Good (60-80% positive): {good_topics} aspects  
- Concerning (<40% positive): {concerning_topics} aspects

Strategic Insight: {"High complexity aspects performing well indicate strong systemic operations" if len(high_complexity) > 0 and high_complexity['pos'].mean() > 0.7 else "Focus on simplifying operations for better performance"}
"""
    except Exception as e:
        return f"Aspect relationship analysis: {len(summary_df)} aspects analyzed with basic categorization available."
...
st.set_page_config(page_title="NLI Multi-Label Business Intelligence Dashboard", page_icon="📊", layout="wide")
...
st.markdown('<h1 class="main-header">🎯 NLI Multi-Label Business Intelligence Dashboard</h1>', unsafe_allow_html=True)
...
if data_source == "Run new pipeline":
    ...
    st.sidebar.subheader("🎯 NLI Multi-Label Analysis Settings")
    ...
    # NLI model parameters
    with st.sidebar.expander("🔧 Advanced NLI Settings"):
        nli_model_name = st.selectbox(
            "NLI Model", 
            ["facebook/bart-large-mnli", "microsoft/DialoGPT-medium", "roberta-large-mnli"],
            index=0
        )
        ...
        local_files_only = st.checkbox("Use cached models only", value=True)
    
    # Output settings
    with st.sidebar.expander("📁 Output Settings"):
        out_dir = st.text_input("Output directory", value="./outputs_nli")  # NLI로 변경
    ...
    # Run pipeline button
    if st.sidebar.button("🚀 Run NLI Analysis Pipeline", type="primary"):
        with st.status("Running NLI analysis pipeline...", expanded=True) as status:
            try:
                ...
                # Step 3: NLI Analysis setup
                st.write("📊 Preparing NLI multi-label analysis...")
                ...
                # NLI analysis runner function
                def run_nli_analysis(scope: str, business_id, out_base_path: Path):
                    apply_nli_multilabel_for_business(
                ...
                if topic_scope in ("per-store", "both"):
                    st.write("📄 Running per-store NLI multi-label analysis...")
                    modeled_count = 0
                    skipped_count = 0
                    ...
                    for i, bid in enumerate(biz_ids):
                        ...
                        try:
                            ...
                            base = run_nli_analysis("per-store", bid, store_path)
                    ...
                    st.write(f"✅ Per-store analysis complete: {modeled_count} analyzed, {skipped_count} skipped")
                
                # Step 5: Save run index and final verification
                if write_index and run_index:
                    idx_path = out_path / "nli_run_index.csv"
                ...
                status.update(label="✅ NLI Pipeline completed successfully!", state="complete")
                ...
                if summary_files and sentence_files:
                    ...
                    try:
                        ...
                        st.balloons()
                        st.success(f"🎉 NLI Pipeline complete! Auto-loaded {len(summary_df)} aspects, {len(sentences_df)} sentences")
                ...
                st.error(f"❌ NLI Pipeline failed: {str(e)}")
                st.exception(e)
else:
...
if st.session_state.get('data_loaded', False):
    ...
    st.markdown("---")
    st.markdown('<h1 style="color: #1f77b4; text-align: center;">📊 NLI Multi-Label Analysis Results Dashboard</h1>', unsafe_allow_html=True)
    ...
    if load_info:
        ...
        <div style='padding: 1rem; background: linear-gradient(90deg, #667eea 0%, #764ba2 100%); border-radius: 10px; margin-bottom: 2rem; color: white; text-align: center;'>
            <h3 style='margin: 0; color: white;'>📊 NLI Analysis Complete!</h3>
            <p style='margin: 0.5rem 0; font-size: 1.1rem;'>
                <strong>{load_info.get('topics_count', 0)} aspects</strong> found in 
    ...
    with col1:
        st.markdown('<h2 class="section-header">🎯 Multi-Label Aspect Performance Analysis</h2>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        st.metric("Total Aspects", total_topics)
        st.metric("Problematic Aspects", problematic_topics, delta=f"{problematic_topics/total_topics:.1%}" if total_topics > 0 else "0%")
        st.metric("Excellent Aspects", excellent_topics, delta=f"{excellent_topics/total_topics:.1%}" if total_topics > 0 else "0%")
        
        # Top keywords
        st.subheader("🔍 Top Aspects")
        
        # Extract individual aspects from NLI labels
        all_aspects = []
    ...
    with st.expander("📊 Detailed Multi-Label Aspect Analysis", expanded=False):
        st.subheader("Complete NLI Multi-Label Aspect Summary")
        ...
        st.dataframe(
            ...
            use_container_width=True,
            column_config={
                "topic_id": st.column_config.NumberColumn("Aspect ID", width="small"),
                "display_label": st.column_config.TextColumn("Multi-Label Aspect", width="large"),
                "label_count": st.column_config.NumberColumn("# Labels", width="small"),
        ...
        st.subheader("Complete NLI Label Breakdown")
        selected_topics = st.multiselect(
            "Select aspects to see all NLI labels (no truncation):",
            options=display_df['topic_id'].tolist(),
            default=display_df.nlargest(3, 'n')['topic_id'].tolist(),
            format_func=lambda x: f"Aspect {x}: {display_df[display_df['topic_id']==x]['compact_label'].iloc[0]}"
        )
        ...
        for topic_id in selected_topics:
            ...
            with st.container():
                st.markdown(f"### Aspect {topic_id}: Complete Multi-Label Analysis")
                st.markdown(f"**{topic_row['display_label']}**")
                st.markdown(f"*NLI classification identified {len(all_aspects)} interconnected labels*")
                
                # Display ALL aspects in a comprehensive format
                st.markdown("**All NLI Labels (Complete List):**")
                ...
                with col5:
                    st.metric("Complexity", f"{len(all_aspects)} labels")
                
                st.markdown("---")
        
        st.subheader("Sample Sentences by Multi-Label Aspect")
        ...
        for _, topic_row in display_df_sorted.head(5).iterrows():
            ...
            with st.container():
                ...
                st.markdown(f"**{display_label}** (Aspect {topic_id})")
                st.caption(f"Encompasses {len(all_aspects)} labels: {', '.join([a.split('(')[0].strip() for a in all_aspects[:3]])}{'...' if len(all_aspects) > 3 else ''}")
                ...
                with st.expander(f"View all {len(all_aspects)} labels for this aspect"):
                    st.markdown("**Complete NLI Label List:**")
                    for i, aspect in enumerate(all_aspects, 1):
                        st.write(f"{i}. {aspect}")
    ...
    with col1:
        ...
        st.download_button(
            label="📊 Download Aspect Summary",
            data=summary_csv,
            file_name=f"nli_aspect_summary_{pd.Timestamp.now().strftime('%Y%m%d_%H%M')}.csv",
            mime="text/csv"
        )
    ...
    with col2:
        ...
        st.download_button(
            ...
            data=sentences_csv,
            file_name=f"nli_sentences_sample_{pd.Timestamp.now().strftime('%Y%m%d_%H%M')}.csv",
            mime="text/csv"
        )
    ...
    with col3:
        ...
        if 'llm_insights' in st.session_state:
            st.download_button(
                ...
                file_name=f"nli_ai_insights_{pd.Timestamp.now().strftime('%Y%m%d_%H%M')}.md",
                mime="text/markdown"
            )
        else:
    ...
    if st.session_state.get('pipeline_running', False):
        st.info("📄 NLI Pipeline is running... Results will appear here when complete.")
    else:
        st.info("👆 Configure and run the NLI pipeline above to see analysis results.")
    ...
    <div style='text-align: center; padding: 3rem; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 10px; color: white; margin: 2rem 0;'>
        <h2>🚀 Welcome to NLI Multi-Label Business Intelligence Dashboard</h2>
        <p style='font-size: 1.2rem; margin: 1rem 0;'>
            Analyze customer sentiment and discover actionable insights using advanced NLI multi-label classification
        </p>
        <p>👈 Use the sidebar to load your analysis results or run a new NLI pipeline</p>
    </div>
    """, unsafe_allow_html=True)
...
st.markdown("---")
st.markdown(
    "<div style='text-align: center; color: #666;'>NLI Multi-Label Business Intelligence Dashboard v2.0 | "
    "Powered by NLI Multi-Label & Streamlit</div>", 
    unsafe_allow_html=True
)